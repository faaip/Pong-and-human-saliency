...
def rollout_human(model, frames, ep_len=1e3, render=False):
    history = {'ins': [], 'logits': [], 'values': [], 'outs': [], 'hx': [], 'cx': []}
    
    state = torch.Tensor(prepro(frames[0])) # get first state
    episode_length, epr, eploss, done  = 0, 0, 0, False # bookkeeping
    hx, cx = Variable(torch.zeros(1, 256)), Variable(torch.zeros(1, 256))

    while episode_length < min(frames.shape[0], ep_len):
        episode_length += 1
